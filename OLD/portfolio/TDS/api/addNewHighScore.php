...
$stmt = $mysqli->prepare("SELECT ID FROM topDownGame WHERE gamePlayer = ?");
...
if($numRows == 1){
	...
	$stmt = $mysqli->prepare("UPDATE topDownGame SET gameScore = ? WHERE ID = ?");
	...
	$stmt = $mysqli->prepare("INSERT INTO topDownGame(gameScore, gamePlayer) VALUES (?, ?)");
